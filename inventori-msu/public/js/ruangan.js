...
function initCards() {
  document.querySelectorAll('.item-card').forEach(card => {
    // Ruangan stock default 1
    card.dataset.max = 1;

    // Cek apakah sudah ada di cart
    updateBadgeAndButtons(card);
  });
}
initCards();

function updateBadgeAndButtons(card) {
  ...
  let inCart = false;
  if (window.MSUCart) {
    inCart = MSUCart.has(name, 'ruang');
  }

  // Jika inCart → berarti "Selected" / "Habis" (karena stok cuma 1)
  if (badge) {
    if (inCart) {
      badge.textContent = 'Selected';
      badge.style.background = '#167c73'; // Tetap hijau/active style
    } else {
  ...
  // Tombol logic:
  // Jika sudah di cart: (-) Enabled (untuk batal), (+) Disabled
  // Jika belum di cart: (-) Disabled, (+) Enabled
  if (minusBtn) minusBtn.disabled = !inCart;
  if (plusBtn) plusBtn.disabled = inCart;
}
...
function confirmAddNoRedirect() {
  ...
  // Visual update only
  updateBadgeAndButtons(card); // Will be updated after adding to cart
  ...
  try {
    if (window.MSUCart) {
      // Enforce limit 1 for rooms
      MSUCart.add(name, 'ruang', thumb, 1, 1);
      MSUCart.renderBadge();
    }
  } catch (e) { }

  // Re-check state after adding
  updateBadgeAndButtons(card);
...
document.addEventListener('click', (e) => {
  ...
  // DO NOT change sisaEl text, it is Capacity!
  ...
  // − : batalkan (remove from cart)
  if (window.MSUCart) {
    // Set qty 0 deletes it
    MSUCart.upsertItem({ type: 'ruang', name: name, qty: 0 });
    MSUCart.renderBadge();
  }
  updateBadgeAndButtons(card);
});
