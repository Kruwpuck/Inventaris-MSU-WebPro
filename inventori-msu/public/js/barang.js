...
// ==== Disable Auto Injection (User manually added HTML) ====
...
function getMetaFromDom() {
    return {
        tanggal: document.getElementById('dateStart')?.value || '',
        mulai: document.getElementById('timeStart')?.value || '',
        durasi: document.getElementById('durationSel')?.value || ''
    };
}

// Auto-save meta on Datebar interaction
function initDatebarSync() {
    const inputs = ['dateStart', 'timeStart', 'durationSel'];
    inputs.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            el.addEventListener('change', () => saveBookingMeta(getMetaFromDom()));
            el.addEventListener('input', () => saveBookingMeta(getMetaFromDom()));
        }
    });

    const btnCheck = document.getElementById('btnSetDates');
    if (btnCheck) {
        btnCheck.addEventListener('click', () => {
            saveBookingMeta(getMetaFromDom());
            // Optional: Visual feedback or scroll to results
            showToastSuccess('Waktu peminjaman disimpan.');
        });
    }

    // Load saved meta on page load to restore state (if returning to home)
    const saved = loadBookingMeta();
    if (saved.tanggal) {
        const d = document.getElementById('dateStart');
        if (d) d.value = saved.tanggal;
    }
    if (saved.mulai) {
        const t = document.getElementById('timeStart');
        if (t) t.value = saved.mulai;
    }
    if (saved.durasi) {
        const du = document.getElementById('durationSel');
        if (du) du.value = saved.durasi;
    }
}
window.addEventListener('load', initDatebarSync);
document.addEventListener('livewire:navigated', initDatebarSync);
...
function initSearch() {
    ...
    // Clean up old listeners (simple approach: clone node to wipe listeners, or just rely on new DOM)
    // Since Livewire replaces DOM, we are attaching to FRESH elements nicely.
    q.addEventListener('input', applyFilter);
    if (clearBtn) {
        clearBtn.onclick = () => { // use onclick to overwrite potential old handlers if node reused
            q.value = '';
            applyFilter();
            q.focus();
        };
...
// Hook into all relevant lifecycle events
