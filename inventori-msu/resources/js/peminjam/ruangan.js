...
function confirmAddNoRedirect() {
  ...
  // pilih ruang (sisa -> 0)
  const sisaEl = card.querySelector('.sisa');
  let sisa = 1; // Default assumes available if clicking add
  // Visual update (toggle)
  if (sisaEl) {
    // Don't trust text content if it looks like capacity (big number)
    // Just force toggle logic
    sisa = 0;
    sisaEl.textContent = '0';
  }
  updateBadgeAndButtons(card, 0);
...
document.addEventListener('click', (e) => {
  ...
  // − : batalkan
  if (sisaEl) sisaEl.textContent = '1';
  updateBadgeAndButtons(card, 1);
});
