...
(function initDateBar() {
  ...
  btnSet?.addEventListener('click', () => {
    ...
    // Hanya Wajib Tanggal Pakai
    if (!s) {
      showToastInfo('Pilih tanggal pakai terlebih dahulu.');
      return;
    }

    // Jika ada input tanggal akhir tapi validasinya error (sebelum start)
    if (e && e < s) {
      showToastInfo('Tanggal kembali tidak boleh lebih awal dari tanggal pakai.');
      return;
    }

    // Jika End kosong (Datebar V2 atau user tidak isi), samakan dengan Start
    if (!e) e = s;

    // Simpan ke MSUDates & LocalStorage meta
    window.MSUDates.set({ start: s, end: e, time: t, duration: dur });

    try {
      localStorage.setItem('msu_booking_meta', JSON.stringify({ tanggal: s, mulai: t, durasi: dur }));
    } catch (err) { }
